  ...
  HRESULT STDMETHODCALLTYPE DxgiSwapChain::ResizeTarget(const DXGI_MODE_DESC* pNewTargetParameters) {
    ...
    if (m_descFs.Windowed) {
      ...
      if (m_desc.Flags & DXGI_SWAP_CHAIN_FLAG_ALLOW_MODE_SWITCH) {
        ...
        // Mirrors what would happen from the WSI abstraction in
        // ChangeDisplayMode to make the window fill the screen
        // even when we are not setting the mode.
        // We let the wsi abstraction manage it when setting modes
        // to deal with the details of that across platforms.
        DXGI_OUTPUT_DESC desc;
        output->GetDesc(&desc);
        
        RECT newRect = desc.DesktopCoordinates;
        
        wsi::moveWindow(m_window, newRect);
      }
    }
